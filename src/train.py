...
def main():
    ...
    logger.info(f"Loading config from {config['cnf_file']}")
    ...
    dm:LightningDataModule = DataModule(**data_cnf, device=device)
    dm.prepare_data()
    dm.setup(stage="fit")
    ...
    if chkpt_available:
        ...
        model:LightningModule = ACEMol(**model_cnf, tokenizer=dm.tokenizer, root_dir=base_cnf['models_dir'])
    ...
    callbacks.append(ModelCheckpoint(
        ...
        monitor="val/loss",
        save_last=True,
        filename="model-{epoch:02d}-val_loss={val/loss:.4f}",
    ...
    wandb_logger = WandbLogger(project='acemol', save_dir=model_root / Path('logs'), id=id)

    trainer = Trainer(**train_cnf['trainer'], logger = wandb_logger, default_root_dir=model_root, callbacks=callbacks, gradient_clip_val=0.5)
